...
# --------------------------------------------------------------------------- #
# W16: the agentic dispute-investigation endpoint
# --------------------------------------------------------------------------- #
class InvestigateRequest(BaseModel):
    complaint: str = Field(min_length=1, max_length=4000)
    verify: bool = Field(default=True, description="Run the verifier sub-agent.")


class InvestigateResponse(BaseModel):
    outcome: Literal["resolved", "needs_input", "escalated", "exhausted", "failed"]
    answer: str
    findings: list[str] = Field(default_factory=list)
    citations: list[str] = Field(default_factory=list)
    steps: list[dict] = Field(default_factory=list, description="The trajectory, one entry per action.")
    iterations: int = 0
